...
function reportesJS() {
    ...
    if (incidencia == '' || desarrollar_incidencia == '') {
        swal("Error", "Tienes que rellenar todos los datos", "error");
        return false;
        ...
        if (ajax.readyState == 4 && ajax.status == 200) {
            ...
            if (ajax.readyState == 4 && ajax.status == 200) {
                ...
                if (respuesta.resultado == 'OK') {
                    swal("Reporte enviado", "Hemos recibido tu reporte, enseguida nos pondremos a revisarlo.", "success");
                } else {
                    swal("Oops", "Parece que ha habido un error, inténtalo de nuevo.", "error");
                }
